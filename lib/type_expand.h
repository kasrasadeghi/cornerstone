...
Texp Stmt(const Texp& texp, const Texp& proof)
  {
    ...
    UnionMatch(g, "Stmt", texp, proof, {
      ...
      {"Store",  [&](const Texp& t, const Texp& p) {
        ...
        auto content = content_type_value[1];

        auto loc_type_value = Value(t[1], p[1]);
        assert(loc_type_value.value == "type-value");
        auto loc_type = loc_type_value[0];
        auto loc = loc_type_value[1];
        ...
        auto unified_type = ([&](){
          ...
          CHECK(storage_type == content_type.value, "cannot store '" + content_type.value + "' in '" + storage_type + "' storage in " + env.curr_def_name->paren());
          return storage_type;
        })();

        this_stmt = {t.value, {t[0], unified_type, t[1]}};
      }},
